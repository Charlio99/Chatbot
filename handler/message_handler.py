...
class UserLikes:
    ...
    def __init__(self):  # Declare the constructor with or without parameters
        ...
        for aux in list(Decision.get_instance().graph.nodes):
        ...
        CategoryDecision.get_instance().set_option(self.option)
    ...
    def replay_all_keyboard_makeup(self):
        ...
        for node_graph in list(Decision.get_instance().graph.nodes):
            node = Decision.get_instance().graph.nodes[node_graph]['node']
...
# filter on a specific message
@bot.message_handler(
    func=lambda message: users.get_user_by_id(message.chat.id).step == START and 0.8 <= SequenceMatcher(
...
@bot.message_handler(func=lambda message: users.get_user_by_id(message.chat.id).step == NEXT_DECISION,
                     content_types=['text'])
def this_or_that(m):
    cid = m.chat.id
    user_id = users.get_user_by_id(cid)
...
# if the user has issued the "/configure" command, process the answer
@bot.message_handler(func=lambda message: users.get_user_by_id(message.chat.id).step == LOCATION,
...
@bot.message_handler(func=lambda message: users.get_user_by_id(message.chat.id).step == NEW_LOCATION,
                     content_types=['text'])
...
@bot.message_handler(func=lambda message: users.get_user_by_id(message.chat.id).step == RECOMMENDATIONS,
                     content_types=['text'])
...
# filter on a specific message
@bot.message_handler(func=lambda message: users.get_user_by_id(message.chat.id).step == START and
                                          (check_similarity_percentage(message.text, "adiós") or
                                           check_similarity_percentage(message.text, "cancelar")),
                     content_types=['text'])
def command_text_bye(m):
    time.sleep(0.5)
    bot.send_message(m.chat.id, "Adiós, nos vemos pronto", reply_markup=types.ReplyKeyboardRemove())
    bot.send_animation(m.chat.id, 'https://reygif.com/media/pocahontas-saludo-83409.gif', duration=None, caption=None,
                       reply_to_message_id=None, reply_markup=None, parse_mode=None, disable_notification=None,
                       timeout=None)
    ...
    if decision.end == 1:
        ...
        if category is not None:
            if category == 'chefbot':
                ...
                bot.send_sticker(m.chat.id, CHEF, reply_markup=types.ReplyKeyboardRemove())
                time.sleep(0.5)
                end_message(m, user_id)
        ...
        bot.send_message(m.chat.id, "😧", parse_mode="Markdown", reply_markup=types.ReplyKeyboardRemove())
        users.store_step(user_id, START)
...
def end_message(m, user_id):
    ...
    bot.send_sticker(m.chat.id, PEPE_CLAP, reply_markup=types.ReplyKeyboardRemove())
    counter = 0
    users.store_step(user_id, START)
...
def cancel_action(m):
    users.store_step(users.get_user_by_id(m.chat.id), START)
